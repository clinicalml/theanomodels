...
parser.add_argument('-vm','--var_model', action='store',default = 'lstmr', help='Variational Model', type=str, choices=['lstm','lstmlr','lstmr','lstmlr_inp'])
...
parser.add_argument('-ttype','--transition_type', action='store', default = 'simple_gated', help='Layers in transition fxn', type=str, choices=['mlp','simple_gated','gated','linear'])
parser.add_argument('-previnp','--use_prev_input', action='store_true', help='Use previous input in transition')
parser.add_argument('-usenade','--use_nade', action='store_true', help='Use NADE Emission')

parser.add_argument('-el','--emission_layers', action='store',default = 2, help='Layers in emission fxn', type=int)
parser.add_argument('-etype','--emission_type', action='store',default = 'mlp', help='Type of emission fxn', type=str, choices=['mlp','rnn','linear','conditional','conditional_linear'])
parser.add_argument('-mixK','--mixture_components', action='store',default = 20, help='# Mixture components')
...
parser.add_argument('-vonly','--validate_only', action='store_true', help='Only build functions for validations (Faster)')
...
#Medical params
parser.add_argument('-ecomb','--emission_combine', type=str,action='store',default = 'concat',help='Shuffle during training',choices=['sum','concat'])
parser.add_argument('-tcomb','--transition_combine', type=str, action='store',default='concat',help='How to combine transition w/ actions',choices=['sum','concat'])
